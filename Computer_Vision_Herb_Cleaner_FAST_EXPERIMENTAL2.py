...
while True:
    ...
    bankScreen = wincap.get_screen_position(bankWindowConfidence[0])
    ...
    #dump clean herbs
    clean_herbScreen = wincap.get_screen_position(clean_herbConfidence[0])
    ...
    dirty_bank_herbScreen = wincap.get_screen_position(dirty_bank_herbConfidence[0])
    dirty_bank_herbClickPoint = dirty_bank_herb_action.click(dirty_bank_herbScreen, speed = .65)
    ...
    #7 click bank exit
    bank_xScreen = wincap.get_screen_position(bank_xWindowConfidence[0])
    ...
    #time.sleep(.5) #waiting to exit bank
    ...
    if bankWindowConfidence[1] < bank_entrance_threshold:
        ...
        print('num bank = %s/1 | confidence best bank = %s | WINDOWCOORDS(!) best bank =  %s' % (len(bankWindowAllPoints), bankWindowConfidence[1], bankWindowConfidence[0]))
