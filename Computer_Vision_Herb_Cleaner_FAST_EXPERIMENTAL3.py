...
dirty_bank_herb_vision = Vision('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\HerbCleaner\\image library\\dirty_bank_herb.png')
...
while True:
    ...
    bankWindow = bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles')
    bankWindowConfidence = bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'confidence')
    bankWindowAllPoints = bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'allPoints')
    #check it found bank
    if bankWindowConfidence[1] < DETECTION_THRESHOLD:
        print('bankWindow confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(bankWindowConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    screenshot = wincap.get_screenshot()
    in_bankWindow = in_bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles')
    in_bankConfidence = in_bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'confidence')
    if in_bankConfidence[1] < DETECTION_THRESHOLD:
        print('in_bank confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(in_bankConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    if clean_herbConfidence[1] < DETECTION_THRESHOLD:
        print('clean_herb confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(clean_herbConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    #4 find dirty herbs
    screenshot = wincap.get_screenshot()
    dirty_bank_herbWindow = dirty_bank_herb_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles')
    dirty_bank_herbConfidence = dirty_herb_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'confidence')
    dirty_bank_herbWindowAllPoints = dirty_herb_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'allPoints')
    if dirty_bank_herbConfidence[1] < DETECTION_THRESHOLD:
        print('dirty_bank_herb confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(dirty_bank_herbConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    #6 find bank exit
    screenshot = wincap.get_screenshot()
    bank_xWindow = bank_x_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles')
    bank_xWindowConfidence = bank_x_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'confidence')
    bank_xWindowAllPoints = bank_x_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'allPoints')
    if bank_xWindowConfidence[1] < DETECTION_THRESHOLD:
        print('bank_x confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(bank_xWindowConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    bankWindowAllPoints = bank_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'allPoints')
    if bankWindowConfidence[1] < DETECTION_THRESHOLD:
        print('bank entrance confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(bankWindowConfidence[1], DETECTION_THRESHOLD))
        exit()
    else: 
    ...
    screenshot = wincap.get_screenshot()
    dirty_herbWindow = dirty_herb_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles')
    ...
    dirty_herbConfidence = dirty_herb_vision.find(screenshot, DETECTION_THRESHOLD, debug_mode='rectangles', return_mode = 'confidence')
    if dirty_herbConfidence[1] < DETECTION_THRESHOLD:
        print('dirty herb confidence = %s | DETECTION_THRESHOLD = %s | exiting...' %(dirty_herbConfidence[1], DETECTION_THRESHOLD))
        exit()
    ...
    for herb in dirty_herbWindowAllPoints:
        ...
        dirty_herbClickPoint = dirty_herb_action.click(dirty_herbScreen,speed=.5)
